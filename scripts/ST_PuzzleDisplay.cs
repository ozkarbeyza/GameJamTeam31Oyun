    ...
    public IEnumerator CheckForComplete()
    {
        while (!Complete)
        {
            ...
            for (int j = Height - 1; j >= 0; j--)
            {
                for (int i = 0; i < Width; i++)
                {
                    if (!TileDisplayArray[i, j].GetComponent<ST_PuzzleTile>().CorrectLocation)
                    {
                        Complete = false;
                    }
                }
        ...
        if (Complete)
        {
            ...
            // Optionally destroy the puzzle object before loading the next scene
            Destroy(this.gameObject);  // Destroy the puzzle object (this GameObject)

            Debug.Log("Loading LaboratoryScene...");
            Time.timeScale = 1f; // Just in case it's frozen
            SceneManager.LoadScene("LaboratoryScene"); // Make sure this name matches exactly!
        }
    }
